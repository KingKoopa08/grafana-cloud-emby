...
class EmbyExporter:
    ...
    def collect_users(self):
        ...
        for user in users:
            ...
            # Get user activity
            activity = self._make_request(f'/Users/{user_id}/Items/Latest')
            if activity:
    ...
    def collect_libraries(self):
        ...
        for library in libraries:
            ...
            if items and 'Items' in items:
                ...
                # Calculate library size (if available)
                total_size = sum(item.get('Size', 0) for item in items['Items'] if 'Size' in item)
                library_size.labels(library_name=library_name).set(total_size)
                
                # Count recently added items
                for item in items['Items']:
                    if 'DateCreated' in item:
                        created = datetime.fromisoformat(item['DateCreated'].replace('Z', '+00:00'))
                        if (datetime.now(created.tzinfo) - created).days < 1:
                            total_recent += 1
    ...
    def collect_devices(self):
        ...
        if not devices and not isinstance(devices, dict):
            return
        ...
        for device in device_items:
            if device.get('DateLastActivity'):
                last_activity = datetime.fromisoformat(device['DateLastActivity'].replace('Z', '+00:00'))
                if (datetime.now(last_activity.tzinfo) - last_activity).days < 7:  # Active in last week
    ...
    def collect_playback_stats(self):
        """Collect playback statistics."""
        # Get playback reports if available
        stats = self._make_request('/Reports/Activities')
        if stats and 'Items' in stats:
            for activity in stats['Items']:
                if activity.get('Type') == 'PlaybackStart':
                    media_type = activity.get('ItemType', 'unknown')
                    playback_starts.labels(media_type=media_type).inc()
                elif activity.get('Type') == 'PlaybackStopped':
                    media_type = activity.get('ItemType', 'unknown')
                    playback_stops.labels(media_type=media_type).inc()
